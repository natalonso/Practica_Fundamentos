...
**- Variable sqtft_basement **: es numérica.  
...
**- Variable view**: es numérica.Toma valores del 0 al 4. 
...
**- Variable condition**: es numérica. Toma valores del 1 al 5.
...
```{r}
#Dibujamos cómo queda categorizada enfrentada con la variable "log_price"
ggplot(datos_train, aes(x=as.factor(datos_train$yr_renovated_catg), y=log_price, fill=as.factor(datos_train$yr_renovated_catg))) + geom_boxplot() + labs(x="No renovación/ Renovación")+theme(legend.position="none")
```
...
![Figura 1](Casa con 33 habitaciones_v2.jpg){width=90%}
