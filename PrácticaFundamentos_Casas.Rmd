...
**ESTO HAY QUE MODIFICARLO CUANDO TENGAMOS EL DEFINITIVO**
...
Para el análisis de los datos, se han dividido las variables en tres categorías:
  ...
  -**Variables de características de las casas**: bedrooms, bathrooms,floors, sqft_living, sqft_lot, sqft_above, sqfr_basement, sqft_living15 y sqft_lot15.
...
**- Variable  floors**: hay que tener en cuenta que se contabiliza 0.5 plantas cuando una planta no tiene todo el espacio construido como por ejemplor la buhardilla, es decir, una casa que tenga una planta y una buhardilla va a tomar el valor de 1.5. Esta variable es numérica.

```{r}
table(datos_train$floors)
``` 

Las casas tienen entre 1 planta 3.5 plantas, aunque la mayoría de ellas tienen entre 1 y 2 plantas.
...
Podemos observar cómo aparenteme el número de plantas no influye en el precio de las casas.

**- Variable sqtft_basement **: es numérica, pero la vamos a hacer categórica.  
...
Se puede obervar que ambas categorías prácticamente siguen la misma distrución pero las casas que tienen sótano pueden llegar a ser más caras.
...
Se observa que la mayoría de las casas no tienen buenas vistas y que las casas que tienen mejores vistas son más caras respeto de las otras.
...
La mayoría de casas tiene condición 3 y 4.
...
**- sqft_above  **: es numérica. Esta variable es la diferencia entre sqft_living y sqft_basement.
...
**- yr_built  **: es numérica.

```{r}
table(datos_train$yr_built)
yr_built<-as.factor(datos_train$yr_built)
```
...
```
 ...
 Se observa lo siguiente:
   ...
   -  En el caso de **zipcode** podríamos agruparlo por códigos postales, aunque si observamos la correlación con el precio no es indiquitava, ya que tiene un valor negativo próximo a cero.
...
A continuación vamos a realizar la imputación de dato faltante con regresión simple:

```{r}
#datos_mice<-datos_train_aux1[,c(3:12,15,16)]
#log_price,bedrooms,bathrooms_group,log_sqft_lin¡ving,sqftlot,floors,sqft_above,lat,long
...
```{r}
indices_cero_habitaciones<-datos_train$posicion[datos_train$bedrooms==0]
datos_train<-datos_train[-indices_cero_habitaciones,]

indices_cero_banos<-datos_train$posicion[datos_train$bathrooms_group==1]
...
```{r}
# pendiente hacer la correlacion con todas: pasando a numeric
...
Para este apartado nos hemos basado en la matriz de correlaciones del apartado anterior y en los gráficos del análisis exploratorio.
  ...
  -**Modelo1**: hemos añadido las variables que mayor correlación tienen con nuestra variable respuesta, log_sqft_living (0.65), bathrooms_group(0.49) y lat (0.46) y las que con los gráficos se visualizaba que el precio aumentaba grade_categ.
  El **Modelo1** es capaz de explicar casi el 67%  de la variabilidad observada en el precio de las casas (R-squared: 0.6699 ). El valor de $R^2$-ajustado es muy  cercano al $R^2$ (Adjusted R-squared: 0.6697 ) lo que indica que el modelo contiene predictores útiles.

```{r}

modelo1<-lm(formula = log_price ~ log_sqft_living + bathrooms_group + grade_categ + lat, data = datos_train_model)
summary(modelo1)
# R_squared = 0.669, 4 variables
```
...
```
  ...
  -**Modelo3**: hemos añadido las variables que mayor correlación tienen con nuestra variable respuesta, log_sqft_living (0.65), bathrooms_group(0.49) y lat (0.46) y las que con los gráficos se visualizaba que el precio aumentaba grade_categ Y view.
   El **Modelo3** es capaz de explicar casi el 70%  de la variabilidad observada en el precio de las casas (R-squared: 0.6982 ). El valor de $R^2$-ajustado es muy  cercano al $R^2$ (Adjusted R-squared: 0.698) lo que indica que el modelo contiene predictores útiles. 
  
```{r}
modelo3<-lm(formula = log_price ~ log_sqft_living + bathrooms_group + grade_categ + lat + view, data = datos_train_model)
summary(modelo3)
# R_squared = 0.698, 5 variables


# waterfront y view, teniendo el resto de variables iguales afecta 0.01
# bedrooms y bathrooms_group, teniendo el resto de variables iguales afecta 0.01
# yr_renovated_catg no suma al r2ajustado
# log_sqft_living y sqft_living15, teniendo el resto de variables iguales afecta 0.01
# long no aporta

```

**Conclusión**: Finalmente hemos seleccionado el **Modelo2**, ya que hemos ido haciendo pruebas con las distintas variables que pensabamos que podían aportar valor al modelo pero hemos observado que el cambio es mínimo (0.01).

Aunque el modelo3 tiene un $R^2$ mayor, estamos introduciendo más variables y como hemos comentado anteriormente apenas sube el coeficiente (0.01), por lo que elegimos el modelo más sencillo.


### Estudio de colinealidad (VIF)

A continuación vamos a usar el **Factor de Inflación de la Varianza (VIF)**, para ver si existe colinealidad emtre las variables seleccionadas para el modelo. Los límites de referencia que se suelen emplear son:
...
Para comprbar si nuestra elección del modelo es la correcta, vamos a usar técnicas de selección automática.
...
```{r}
#por defecto el número de subconjuntos evaluados está limitado a ocho

mejor_modelo_forward <- leaps::regsubsets(log_price~., datos_train_model[,-1], method="forward") #Best subset forward
...
```{r}
mejor_modelo_backward <- leaps::regsubsets(log_price~., datos_train_model[,-1], method="backward") #Best subset backward
...
datos_test_mod <- datos_test[c(1,2,22,4,23,24,7:11,25,13,26,15,27,17:21)]
```

```{r}
#el modelo elegido de momento (porque estaba puesto), es el último
#eliminamos la variable respuesta para predecir
predict_precio_test <- as.data.frame(predict(modelo2, newdata = datos_test_mod[-3,]))
colnames(predict_precio_test)[1] <- "predict_precio_test"
```
...
comparacion <- comparacion[,c(1,4,23)]
```

```{r}
comparacion$predict_precio_test <- comparacion$log_price - comparacion$predict_precio_test
mean(comparacion$predict_precio_test)
```
...
datos_validacion_mod <- datos_validacion[c(1,2,22,4,23,24,7:11,25,13,26,15,27,17:21)]
```

```{r}
#el modelo elegido de momento (porque estaba puesto), es el último
#eliminamos la variable respuesta para predecir
predict_precio_validacion <- as.data.frame(predict(modelo2, newdata = datos_validacion_mod[-3,]))
...
comparacion <- merge(datos_validacion_mod,predict_precio_validacion,by="id_aux")
comparacion <- comparacion[,c(1,4,23)]
```

```{r}
comparacion$dif_precios <- comparacion$log_price - comparacion$precio_pred 
mean(comparacion$dif_precios)
```


Como el resultado es negativo, el precio predicho es mayor que el real, el modelo tiene un pequeño error al alza, el precio predicho de media, es superior al real.