...
Se puede observar cómo crece el precio de las casas a media que aumenta el número de baños por casa.
...
Se puede obervar que más o menos ambas categorías siguen la misma distrución.
...
```{r}
ggpairs(datos_train,columns=c(2,16:18),mapping = aes(color = "red") )

```

**TENEMOS QUE COMENTAR ESTE GRAFICO**
...
modelo5<-lm(formula = log_price ~ log_sqft_living + bathrooms_group + grade_categ + lat + floors + view, data = datos_train_model)
summary(modelo5)
# R_squared = 0.7
...
regfit_full <- leaps::regsubsets(log_price~., datos_train_model[,-1]) #Best subset
regfit_full
