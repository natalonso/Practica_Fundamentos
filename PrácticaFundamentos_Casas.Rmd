...
![Figura1](Casa con 33 habitaciones_v2.jpeg){width=90%}
...
El modelo es capaz de explicar el 70% de la variabilidad observada en el precio de las casas (R-squared: 0.7003 ). El valor de R2-ajustado es muy  cercano al R2 (Adjusted R-squared: 0.7001 ) lo que indica que el modelo contiene predictores útiles. El test F muestra un p-value de 0.22e-15 por lo que el modelo en conjunto es significativo. Esto se corrobora con el p-value de cada predictor, en ambos casos significativo excepto grade_categ1.(Se podría volver a categorizar esta variable)
...
## Creación del modelo por selección automática (Best subset)

El proceso de Best Subset consiste en evaluar todos los posibles modelos que se pueden crear por combinación de los predictores disponibles.

```{r}

#usamos una seleccion automatica de variables para comparar la evaluacion del modelo
#por defecto el número de sbconjuntos evaluados está limitado a ocho
