...
En este aparatado vamos a analizar como se comportan las variables de localización respecto al precio.
...
Antes de crear el modelo, vamos a crear un nuevo dataframe con las variables que vamos a usar y vamos a reescalarlas. Primero vamos a hacer una prueba para ver que tipo de reescalado es el más adecuado, para ello se representan 4 posibles versiones del reescalado sobre la variable respuesta log_price para ver el efecto de cada tipo de escalado. Finalmente, decidimos que el más adecuado para nuestra análisis iba a ser la estandarización (ya que con este tipo de reescalado no nos afecta tanto la presencia de outliers como en la normalización, que depende tanto del valor máximo como mínimo).
...
```
  ...
  Como el modelo 2, será el que finalmente elijamos, vamos a ver las diferencias entre reescalar las variables o no reescalarlas. Vemos que los resultado no varían mucho. (EXPLICAR ESTO UN POCO MAS LO DEL INTERCEPT Y LO DE Residual standard error!!!)
...
summary(modelo2_reescalado) #-----> ELEGIDO!!
...
En el modelo seleccionado vemos que no hay índices altos de colinealidad por lo que la elección de las variables es buena. Además, no hay diferencia entre el modelo 2 con y sin reescalado de variables numéricas.
