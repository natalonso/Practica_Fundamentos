...
datos_train_model <- datos_train[c(1,21,3,22,23,6,7:10,25,12,24,17:20,26,14)]
```
...
datos_train_model$sqft_lot15<-rescale(datos_train_model$sqft_lot15)
...
#modelo1<-lm(formula = log_price ~ log_sqft_living + bathrooms_group + grade_categ + lat + floors + yr_renovated_catg, data = datos_train_model)
#summary(modelo1)
# R_squared = 0.67

#modelo2<-lm(formula = log_price ~ log_sqft_living + bathrooms_group + grade_categ + lat + #yr_built, data = datos_train_model)
#summary(modelo2)
# R_squared = 0.68
...
#modelo4<-lm(formula = log_price ~ log_sqft_living + bathrooms_group + grade_categ + lat + waterfront + view, data = datos_train_model)
#summary(modelo4)
# R_squared = 0.7

modelo5<-lm(formula = log_price ~ log_sqft_living + bathrooms_group + grade_categ + lat + floors + view + yr_built + sqft_living15, data = datos_train_model)
summary(modelo5)
# R_squared = 0.7
...
```{r}
vif(modelo5) #nos da un factor que mida la inflacion de la varianza por tener variable co-lineales
```
...
mejor_modelo <- leaps::regsubsets(log_price~., datos_train_model[,-1]) #Best subset
#regfit_full
reg_sum <- summary(mejor_modelo)
#reg_sum
...
datos_tes_mod <- datos_test[c(1,2,22,4,23,24,7:11,25,13,26,15,27,17:21)]
```

```{r}
#el modelo elegido de momento (porque estaba puesto), es el último
#eliminamos la variable respuesta para predecir
predict_precio <- as.data.frame(predict(modelo3, newdata = datos_tes_mod[-3,]))

colnames(predict_precio)[1] <- "precio_pred"
```

## Variable respuesta test vs valor predicho

```{r}
datos_tes_mod$id_aux <- rownames(datos_tes_mod)
predict_precio$id_aux <- rownames(predict_precio)

comparacion <- merge(datos_tes_mod,predict_precio,by="id_aux")
