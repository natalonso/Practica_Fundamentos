...
Otro modo de imputar el dato faltante es ver por distancia euclídea las casas más cercanas sus característica:

```{r}
#datos_train$diferencia_lat<- abs(datos_train$lat[datos_train$bedrooms==33]-datos_train[,18])
#datos_train$diferencia_long<- abs(datos_train$long[datos_train$bedrooms==33]-datos_train[,19])
#datos_train$diferencia_total <- datos_train$diferencia_lat+datos_train$diferencia_long
#casas_mas_cercanas <- sort(datos_train$diferencia_total,index.return=TRUE)
#indices <- casas_mas_cercanas$ix[2:15]
#mean(datos_train$bedrooms[indices])
