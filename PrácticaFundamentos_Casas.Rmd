...
Una vez obtenida la predicción del precio usando el modelo2, comparamos con el precio real calculando la diferencia entre ambos. Finalmente, hallamos la media de todas las diferencias y comprabamos que de media hay una diferencia de -0.02 aproximadamente. 
...
comparacion$precio_diff <- abs(comparacion$log_price - comparacion$predict_precio) #unidades logaritmicas
dif_media_test_unidades_log <- mean(comparacion$precio_diff) #unidades logaritmicas
dif_media_test_unidades_nat<-10^(dif_media_test_unidades_log) #unidades naturales
dif_media_test_unidades_nat #unidades naturales
 
comparacion$precio_diff2 <- 10^(abs(comparacion$log_price - comparacion$predict_precio)) #unidades naturales
dif_media_test_unidades_nat2<-mean(comparacion$precio_diff2)
dif_media_test_unidades_nat2

comparacion$precio_diff3 <- abs(10^(comparacion$log_price) - 10^(comparacion$predict_precio)) #unidades naturales
dif_media_test_unidades_nat3<-mean(comparacion$precio_diff3)
dif_media_test_unidades_nat3
...
comparacion_val$dif_transformada2 <- 10^(comparacion_val$dif_precios)

dif_media_val<-mean(comparacion_val$dif_precios)
10^(dif_media_val)

dif_media_val2<-mean(comparacion_val$dif_transformada2)
dif_media_val2
```

De nuevo, calculamos la diferencia entre el precio predicho y el real, y hallamos la media. En este caso, la diferencia media es de -0.018, muy similar a la anterior. Podemos concluir que el modelo es bueno. Como en ambos casos hemos obtenido un diferencia media negativa, podría deducirse que el modelo precide precios al alza (por encima del real).
...
- Finalmente, con los datos de test y validación, comprobamos que el modelo seleccionado predice los precios con una diferencia con el precio real de aproximadamente 1$ de media aproximadamente. Podemos concluir que el modelo elegido para la predicción del precio es bastante preciso.
