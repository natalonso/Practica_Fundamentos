...
Mas adelante categorizaremos esta variable.
...
p4<-ggplot(datos_train, aes(x=as.factor(bathrooms), y=price, fill=as.factor(bathrooms))) + geom_boxplot()+
...
#options(scipen=999)
#options(repr.plot.width=6, repr.plot.height=3,align="center")
p18<-ggplot(datos_train, aes(x=yr_built)) + geom_histogram(colour="black",bins=30, fill="#E1AF00") 

p19<-ggplot(datos_train, aes(x=as.factor(yr_built), y=price, fill= as.factor(yr_built))) + geom_boxplot()+
labs(x="yr_built")+theme(legend.position="none")
...
plot2<-ggpairs(datos_train,columns=c(3,9:12,15,16),mapping = aes(color = "red") )
...
```{r}
#Dibujamos como queda categorizada enfrentada con la variable "price"
ggplot(datos_train, aes(x=as.factor(datos_train$yr_renovated_catg), y=log_price, fill=as.factor(datos_train$yr_renovated_catg))) + geom_boxplot() + labs(x="grade_train")+theme(legend.position="none")
```
# Detección, tratamiento e imputación de datos faltantes
...
datos_train2$bedrooms[datos_train2$bedrooms==33] <-NA
#sum(is.na(datos_train))
...
datos_mice<-datos_train2[,c(3:12,15,16)]
...
datos_mice2<-datos_train2[,c(3:12,15,16)]
```

```{r}
# Imputación multiple, regresión estocástica:

imp_2 = mice(datos_mice2, method = "norm.nob")# Cálculo método de imputación
